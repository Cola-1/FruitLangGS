...
def load_ply_data(path, max_sh_degree=3):
    ...
    if "f_dc_0" in plydata.elements[0].data.dtype.names:
        ...
        dc_scale = np.sqrt(4 * np.pi)
        red = np.asarray(plydata.elements[0]["red"]).astype(np.float32)
        green = np.asarray(plydata.elements[0]["green"]).astype(np.float32)
        blue = np.asarray(plydata.elements[0]["blue"]).astype(np.float32)
    
        features_dc = np.zeros((xyz.shape[0], 3, 1), dtype=np.float32)
        features_dc[:, 0, 0] = red / 255.0 * dc_scale
        features_dc[:, 1, 0] = green / 255.0 * dc_scale
        features_dc[:, 2, 0] = blue / 255.0 * dc_scale
