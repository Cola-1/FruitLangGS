    ...
    def split_large_cluster(self, X, C, labels):
        ...
        for cluster in tqdm(X, desc='Large Fruit Clusters processed'):
            ...
            if isinstance(alpha_shape, trimesh.Trimesh):
                mesh = o3d.geometry.TriangleMesh(
                    vertices=o3d.utility.Vector3dVector(alpha_shape.vertices),
                    triangles=o3d.utility.Vector3iVector(alpha_shape.faces)
                )
                alpha_shape_pcd = mesh.sample_points_uniformly(number_of_points=1000)
