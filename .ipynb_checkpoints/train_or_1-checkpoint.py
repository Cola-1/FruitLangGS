...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # Loss and training objective (replaced)
        gt_image = viewpoint_cam.original_image.cuda()
        Ll1 = l1_loss(image, gt_image)
        # base image loss with SSIM weighting
        loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image))
        # load regularization loss
        if load_distribution is not None:
            load_loss = torch.std(load_distribution)
            # normalize magnitudes via log-scale adjustment
        ...
        with torch.no_grad():
            ...
            # Densification and pruning (unchanged)
